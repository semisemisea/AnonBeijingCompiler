...
fn compile_constant_binary(op: BinaryOp, divisor: i32) -> String {
    ...
    let function = program.new_function(Type::get_i32(), "constant".into(), vec![Type::get_i32()]);
...
fn signed_power_of_two_formula_truncates_toward_zero() {
    ...
    for dividend in dividends {
        for divisor in divisors {
            ...
            let remainder = dividend.wrapping_sub(positive_quotient.wrapping_shl(u32::from(shift)));
...
fn constant_mul_folds_to_shift_or_add_shift() {
    ...
    let mut program = Program::new();
    let function = program.new_function(Type::get_i32(), "mul_const".into(), vec![Type::get_i32()]);
    let data = program.func_data_mut(function);
    let entry = data.add_entry_block();
    let x = data.params()[0];
