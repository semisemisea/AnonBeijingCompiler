 ...
 * and optimized.
 *
 * Local adaptation: allocator-backed `Vec2`/`Bump` storage is represented by `Vec`.
 */
...
use crate::reg_alloc::{
    function::Function,
    index::{Block, ContainerComparator},
    reg::{Allocation, Edit, MachineEnv, Operand, Output, PReg, ProgPoint, RegClass, VReg},
};
use log::trace;
...
impl CodeRange {
    ...
    pub fn len(&self) -> usize {
        self.to.inst() as usize - self.from.inst() as usize
    }
...
// The local program-point representation predates regalloc2 index helpers.
impl ProgPoint {
    #[inline(always)]
    pub fn to_index(self) -> u32 {
        self.inst() * 2 + self.pos() as u32
    }
    ...
    pub fn from_index(index: u32) -> Self {
        Self::new(
            index >> 1,
            if index & 1 == 0 {
                crate::reg_alloc::reg::InstPosition::Before
            } else {
                crate::reg_alloc::reg::InstPosition::After
            },
        )
    }
}
...
pub type LiveRangeList = Vec<LiveRangeListEntry>;
pub type UseList = Vec<Use>;
...
impl LiveRanges {
    pub(crate) fn add(&mut self, range: CodeRange) -> LiveRangeIndex {
        self.push(LiveRange {
            range,
...
impl LiveBundles {
    pub(crate) fn add(&mut self) -> LiveBundleIndex {
...
impl Default for Ctx {
    fn default() -> Self {
        Self {
            cfginfo: CFGInfo::default(),
            cfginfo_ctx: CFGInfoCtx::default(),
            liveins: Vec::new(),
            liveouts: Vec::new(),
            blockparam_outs: Vec::new(),
            blockparam_ins: Vec::new(),
            ranges: LiveRanges::default(),
            bundles: LiveBundles::default(),
            spillsets: SpillSets::default(),
            vregs: VRegs::default(),
            pregs: Vec::new(),
            allocation_queue: PrioQueue::default(),
            spilled_bundles: Vec::new(),
            spillslots: Vec::new(),
            slots_by_class: Default::default(),
            extra_spillslots_by_class: Default::default(),
            preferred_victim_by_class: [PReg::invalid(); 3],
            ...
            scratch_vreg_ranges: Vec::new(),
            scratch_spillset_pool: Vec::new(),
            scratch_workqueue: VecDeque::new(),
            scratch_operand_rewrites: FxHashMap::default(),
            scratch_removed_lrs: FxHashSet::default(),
            scratch_removed_lrs_vregs: FxHashSet::default(),
            scratch_workqueue_set: FxHashSet::default(),
        }
    }
}
...
pub struct PrioQueue {
    pub heap: BinaryHeap<PrioQueueEntry>,
}
