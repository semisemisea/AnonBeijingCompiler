 ...
 * for taki_mir's `reg_alloc` paths, `Vec` storage, u32 program points, and
 * `Output`, which has no debug labels, annotations, or statistics.
 */
...
use super::reg_traversal::RegTraversalIter;
use super::{
...
use crate::reg_alloc::{
    ...
    reg::{
        Allocation, InstPosition, OperandConstraint, OperandKind, OperandPos, PReg, ProgPoint,
        RegClass, SpillSlot,
    },
};
use log::trace;
...
impl<F: Function> Env<'_, F> {
    ...
    pub fn apply_allocations_and_insert_moves(&mut self) -> InsertedMoves {
        ...
        fn choose_move_location<F: Function>(
            ...
            if to_ins > 1 && from_outs <= 1 {
                (
                    ...
                    ProgPoint::before(from_last_insn.raw_u32()),
                    InsertMovePrio::OutEdgeMoves,
                )
            } else if to_ins <= 1 {
                (
                    ProgPoint::before(to_first_insn.raw_u32()),
        ...
        for vreg in 0..self.vregs.len() {
            ...
            for range_idx in 0..self.vregs[vreg].ranges.len() {
                ...
                // Source half-move.
                let mut block = self.cfginfo.insn_block[range.from.inst() as usize];
                ...
                for use_idx in 0..self.ranges[entry.index].uses.len() {
                    ...
                    let inst = Inst::new(usedata.pos.inst() as usize);
    ...
    pub fn resolve_inserted_moves(&mut self, mut inserted_moves: InsertedMoves) -> Edits {
        ...
        fn redundant_move_process_side_effects<F: Function>(
            ...
            if this.cfginfo.insn_block[from.inst() as usize]
                != this.cfginfo.insn_block[to.inst() as usize]
            {
