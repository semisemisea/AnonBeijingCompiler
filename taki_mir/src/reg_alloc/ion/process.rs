 ...
 * and optimized.
 *
 * Local adaptation note: copied in full from regalloc2 0.15.1 and adapted
 * only for taki_mir's `reg_alloc` paths, `Vec`-backed Ion storage, u32
 * instruction indices, and output/statistics representation.
 */
...
use crate::reg_alloc::{
    ...
    reg::{Allocation, InstPosition, OperandConstraint, OperandKind, PReg, ProgPoint},
};
use core::fmt::Debug;
use log::trace;
...
impl<F: Function> Env<'_, F> {
    ...
    fn compute_bundle_prio(&self, bundle: LiveBundleIndex) -> u32 {
        self.ctx.bundles[bundle]
            .ranges
            .iter()
            .map(|entry| entry.range.len() as u32)
            .sum::<u32>()
            .max(1)
    }

    /// Derive the tightest register-encoding limit from the bundle's uses.
    fn compute_bundle_limit(&self, bundle: LiveBundleIndex) -> Option<u8> {
        self.ctx.bundles[bundle]
            .ranges
            .iter()
            .flat_map(|entry| self.ctx.ranges[entry.index].uses.iter())
            .filter_map(|u| match u.operand.constraint() {
                OperandConstraint::Limit(limit) => u8::try_from(limit).ok(),
                _ => None,
            })
            .min()
    }
    ...
    pub fn split_and_requeue_bundle(
        ...
        let mut new_lr_list: LiveRangeList = Vec::new();
        new_lr_list.extend(
            self.ctx.bundles[bundle]
                .ranges
                .iter()
        ...
        for entry in core::mem::replace(&mut self.ctx.bundles[bundle].ranges, empty_vec) {
            ...
            for u in core::mem::replace(&mut self.ctx.ranges[entry.index].uses, empty_vec) {
                ...
                if is_def {
                    ...
                    spill_range.from = ProgPoint::before(u.pos.inst() + 1);
                }
    ...
    pub fn process_bundle(&mut self, bundle: LiveBundleIndex, hint: PReg) -> Result<(), String> {
        ...
        'outer: loop {
            ...
            let mut lowest_cost_split_conflict_point = ProgPoint::before(0);
                ...
                match self.try_to_allocate_bundle_to_reg(
                    ...
                    AllocRegResult::Conflict(bundles, first_conflict_point) => {
                        ...
                        let loop_depth =
                            self.ctx.cfginfo.approx_loop_depth[self.ctx.cfginfo.insn_block
                                [first_conflict_point.inst() as usize]
                                .index()];
                    ...
                    AllocRegResult::ConflictWithFixed(max_cost, point) => {
                        ...
                        let loop_depth = self.ctx.cfginfo.approx_loop_depth
                            [self.ctx.cfginfo.insn_block[point.inst() as usize].index()];
                        let move_cost = spill_weight_from_constraint(
                            OperandConstraint::Reg,
                            loop_depth as usize,
                            /* is_def = */ true,
                    ...
                    || lowest_cost_evict_conflict_cost.unwrap() >= our_spill_weight)
            {
                if matches!(req, Requirement::Register | Requirement::Limit(_)) {
                    ...
                    if min_bundles_assigned + fixed_assigned >= total_regs {
                        return Err("too many live registers".to_owned());
                    }
                }
                    ...
                    || our_spill_weight <= lowest_cost_evict_conflict_cost.unwrap())
            {
                ...
                let bundle_start_depth = self.ctx.cfginfo.approx_loop_depth
                    [self.ctx.cfginfo.insn_block[bundle_start.inst() as usize].index()];
                let split_at_depth = self.ctx.cfginfo.approx_loop_depth
                    [self.ctx.cfginfo.insn_block[split_at_point.inst() as usize].index()];
                if split_at_depth > bundle_start_depth {
                    for block in (self.ctx.cfginfo.insn_block[bundle_start.inst() as usize].index()
                        + 1)
                        ..=self.ctx.cfginfo.insn_block[split_at_point.inst() as usize].index()
                    {
...
fn minimal_range_for_use(u: &Use) -> CodeRange {
    ...
    let next_early = ProgPoint::before(u.pos.inst() + 1);
